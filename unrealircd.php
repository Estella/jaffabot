...
$debug = false; if ($debug) {
                   fwrite(STDOUT,"! !WARNING! ! You are running the Unreal server protocol module in DEBUG mode. This means all input and output to the bot is logged to STDOUT.\r\n");
                   ...
                   fwrite(STDOUT,"! !WARNING! ! and edit unrealircd.php to change \$debug = true; to \$debug = false;.\r\n");
}
...
class protocol {
	...
	function protocol_start($args){
		...
		regCallback($this,"irc_njoin","SJOIN");
		...
		regCallback($this,"irc_metadata","SVSMODE");
		regCallback($this,"irc_metadata","SVS2MODE");
		$this->sw(sprintf("PASS %s",$args[1]));
		$this->sw(sprintf("PROTOCTL VL NICKv2 VHP SJOIN SJOIN2 SJ3",$args[1]));
		$this->sw(sprintf("SERVER %s 1 :U2311-Oo %s",$args[0],$args[2]));
	}
	...
	function sendUid($sender,$nick,$ident,$host,$dhost,$uid,$realname="* Unknown *"){
		...
		$this->sw(sprintf("NICK %s 1 %s %s 1 +ioqS :%s", $nick, $ident, $host, $realname));
	...
	function sts_login($nick,$accountname,$vhost) {
		global $confItems, $file, $opMode, $Mline, $protofunc, $mods, $callbacks, $socket, $privcalls, $debug;
		$this->sw(sprintf(":%s SVS2MODE %s +d %s",$confItems["SID"], $nick,$accountname));
		if ($vhost != "") $this->sw(sprintf(":%s CHGHOST %s %s",$confItems["SID"], $nick,$vhost));
