...
class p10n {
	...
	function irc_acct($args) {
		...
		$this->accnt[$args[1]] = $args[2];
		callEvents(array("cmd"=>"ACCOUNT","nick"=>$args[1],"action"=>"I","acct"=>$args[2]));
	}
	...
	function irc_nick($args) {
		...
		if (count($args) > 5) { /* Got the right number of arguments for a user introduction? */
			if (strpos($args[6],"r") !== false) {
				...
				$strip = strpos($args[7],":");
				$this->accnt[$num] = strtolower(substr($args[7],0,$strip));
	...
	function sts_login($nick,$accountname,$vhost) {
		global $confItems, $file, $opMode, $Mline, $protofunc, $mods, $callbacks, $socket, $privcalls, $debug;
		$this->sw(sprintf("%s AC %s %s",$confItems["SID"],$nick,$accountname));
		if ($vhost != "") $this->sw(sprintf("%s SH %s %s %s",$confItems["SID"], $nick, $accountname,$vhost));
		$this->accnt[$nick] = $accountname;
		return true; //user logged in and vhosted
	}

	function sts_sh($nick,$vhost) {
		global $confItems, $file, $opMode, $Mline, $protofunc, $mods, $callbacks, $socket, $privcalls, $debug;
		if ($vhost != "") $this->sw(sprintf("%s SH %s %s %s",$confItems["SID"], $nick,$this->accnt[$nick],$vhost));
		return true; //user logged in and vhosted
	}
	...
	function protocol_privmsg($get){
		...
		foreach ($privcalls[$this->dec(substr($get[1],-3))][$cmd[0]] as $callback) {
			call_user_func($callback,$get["sendernick"],$this->dec(substr($get[1],-3)),$cmd[1]);
		}
